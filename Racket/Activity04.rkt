...
;;(define (multiple7 lst)
 ;; (if (null? lst)
   ;;   null
    ;;  (if (= (remainder (car lst) 7) 0)
      ;;    (cons (car lst) (multiple7 (cdr lst)))
        ;;  (multiple7 (cdr lst))
     ;; )
  ;; )
 ;;)
...
(define (large-countries lst threshold)
        (map car(filter (lambda (x) (> (cadr x) threshold))lst))
...
(define (combine lstA lstB)
         (apply append(map list lstA lstB))	
)
...
(define (sumeven lst)
        (apply + (filter (lambda (x)(= (remainder x 2) 0)) lst))
)
